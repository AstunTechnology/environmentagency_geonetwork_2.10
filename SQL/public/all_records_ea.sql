...
-- DROP VIEW all_records_ea;

CREATE OR REPLACE VIEW all_records_ea AS 
 SELECT bah.id,
    bah.uuid,
    ...
    ('"'::text || array_to_string(xpath('/gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:abstract/gco:CharacterString/text()'::text, bah.data_xml, '{{gco,http://www.isotc211.org/2005/gco},{gmd,http://www.isotc211.org/2005/gmd}}'::text[]), '-'::text)) || '"'::text AS abstract,
   ...
   FROM ( SELECT DISTINCT ON (metadatastatus.metadataid) metadatastatus.metadataid,
     ...
     LEFT JOIN ( SELECT metadata_xml.id,
          ...
          ORDER BY metadata_xml.id) as bah ON foo.metadataid = bah.id
